                ...
                var directUpload = function(up, file, func) {
                    ...
                    if (op.save_key) {
                        ...
                        multipart_params_obj = {
                            ...
                            "AWSAccessKeyId": that.ak,
                            "Policy": that.policy,
                            "Signature": that.token,
                        };
                        var policyJsonStr = atob(that.policy);
                        var policyJson = JSON.parse(policyJsonStr);
                        for (i = 0; i < policyJson.conditions.length; i++) {
                            var item = policyJson.conditions[i];
                            if (typeof item === 'object') {
                                if ('x-amz-algorithm' in item) {
                                    multipart_params_obj['x-amz-algorithm'] = item['x-amz-algorithm']
                                }
                                if ('x-amz-credential' in item) {
                                    multipart_params_obj['x-amz-credential'] = item['x-amz-credential']
                                }
                                if ('x-amz-date' in item) {
                                    multipart_params_obj['x-amz-date'] = item['x-amz-date']
                                }
                            }
                        }
                    }
